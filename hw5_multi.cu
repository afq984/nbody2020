...
template <bool with_devices>
__device__ void run_step_single(int step, int n, Vec3* q, Vec3* nq, Vec3* v, Prop* p) {
    int i = threadIdx.x;
    double ax = 0;
    double ay = 0;
    double az = 0;
...
__global__ void prob1(double* answer, int planet, int asteroid, int n, Vec3* q,
    ...
    for (int step = 0; step < param::n_steps; step++) {
        ...
        run_step_single<false>(step + 1, n, q, nq, v, p);
        {
            Vec3* tmp = q;
            q = nq;
            nq = tmp;
...
int main(int argc, char** argv) {
    ...
    tasks.push_back([&](Vec3* dq, Vec3* dnq, Vec3* dv,
        ...
        prob1<<<1, n>>>(dev_min_dist, planet, asteroid, n, dq, dnq, dv, dp);
